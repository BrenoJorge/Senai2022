...
-- Deletar um índice para o CPF
DROP INDEX i_cpf ON dados_cli;


select nome from dados_cli where cpf="38135794924";
...
insert into itens_pedido values (last_insert_id(), 11, 1, 31.33);
insert into itens_pedido values (last_insert_id(), 14, 2, 30.85);
...
CREATE TRIGGER calcular_pedido after insert on itens_pedido
for each row
BEGIN
    call set_total_pedido(NEW.pedido_id);
END//

delimiter ;

call new_pedido_1item(75, 28, 1);
...
-- select PROVA
select i.valor, f.Nome_Func from itens_solicitacao i order by valor desc limit 1 inner join Solicitacoes s on ;

SELECT p.Nome_produto FROM Solicitacoes s
WHERE Data_sol BETWEEN "2018-02-01" AND "2018-02-28"
inner join Itens_Solicitacao I on;